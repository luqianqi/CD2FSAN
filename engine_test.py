...
def validate_epoch(args, val_loader, model, logger, mode='val'):
    ...
    oiou = AverageMeter()  # 添加oiou的AverageMeter
    ...
    for batch_idx, (imgs, word_id, word_mask, bbox, seg_map, ratio, dw, dh, im_id, phrase, draw_img) in enumerate(val_loader):
        ...
        # 计算每个样本的IoU、交集和并集
        seg_iou, seg_prec, I, U = cal_seg_iou2(seg_map[0].cpu().numpy(), mask_out_np, args.seg_thresh)
        ...
        if batch_idx % 1000 == 0:
            print_str = '[{0}/{1}]\t' \
                        ...
                        'oiu {oiou.val:.4f} ({oiou.avg:.4f})\t' \
                        .format(
                batch_idx, len(val_loader), batch_time=batch_time, seg=miou_seg, oiou=oiou)
            print(print_str)
            logger.info(print_str)
        idx = idx + 1

    # 计算oiou
    oiou_val = cum_I * 100. / cum_U if cum_U != 0 else 0
    oiou.update(oiou_val)

    print(miou_seg.avg)
    print("Overall IoU: {:.4f}".format(oiou.avg))
    for thresh in thresholds:
        print("prec@{}: {:.4f}".format(thresh, float(prec[thresh].avg)))
        logger.info("prec@{}:{}".format(thresh, float(prec[thresh].avg)))
    logger.info("{:.4f},{:.4f}".format(float(miou.avg), miou_seg.avg))
    logger.info("Overall IoU: {:.4f}".format(oiou.avg))
    return miou_seg.avg, prec, oiou.avg  # 返回miou、prec和oiou
